...
const EnhancedROICalculator = () => {
  ...
  // Separate state for business category to ensure it's properly managed
  ...
  // Special handler for business category
  const handleBusinessCategoryChange = useCallback((value) => {
    console.log('Business category changing to:', value);
    setBusinessCategory(value);
    // Clear error when user updates field
    setErrors(prev => {
      if (prev.business_category) {
        const newErrors = { ...prev };
  ...
  const validateStep = (step) => {
    const newErrors = {};
    console.log('Validating step', step, 'with formData:', formData, 'businessCategory:', businessCategory); // Debug log
    ...
    switch (step) {
      ...
      case 2:
        if (!businessCategory || businessCategory === '') {
          newErrors.business_category = 'Business category selection is required';
        }
  ...
  const handleNext = () => {
    ...
    console.log('Current businessCategory:', businessCategory); // Debug log
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Combine formData with businessCategory for submission
      const submissionData = {
        ...formData,
        industry: businessCategory, // Map to industry for backend compatibility
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      <h3 className="text-2xl font-bold text-gray-900 mb-6">Business Category & Performance</h3>
